...
export default function DclHoodie(props) {
  ...
  useEffect(() => {
    if(authenticated){
      axios.get('https://m59jtciqre.execute-api.us-east-1.amazonaws.com/getDclHoodiesNfts', {
       ...
       .then(({data}) => {
         ...
         setNftBalances(data);
         console.log('DCLHoodie Data: ', data);
       }).catch((error) => {
         // Handle errors
         console.error(error);
       });
      ...
      {!authenticated && (
        ...
        <CardActions>
          <Stack direction="row" spacing={2} sx={{ width:'100%'}} justifyContent="space-between">
            ...
            <IconButton
                size="small"
                href={`https://opensea.io/assets/matic/0xCF97C3b8db98733C69B9d6560A27a734c49177Fb/${nft.tokenId}`}
                target="_blank"
                rel="noreferrer"
              >
                 <Box
